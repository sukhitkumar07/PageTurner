...
const BookByName = () => {
  ...
  let search = () => {
    ...
    axios
      .get(`http://localhost:8080/books/find-by-name/${name}`)
      .then((res) => {
        ...
        <tr className="p-2 flex flex-row justify-center items-center rounded-md">
          <td>
            <input
              type="text"
              id="ip"
              placeholder="Enter ctrl + b to find book by name "
              className="w-[320px] h-9 mx-4 outline-none border-y-2 border-blue-950 rounded-lg px-8 "
            />
          </td>
          <td>
            <button
